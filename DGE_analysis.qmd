...
ggplot(top_genes_plot_data, aes(y = Gene_Sorted, x = Counts, fill = Gene)) +
  ...
  geom_text(aes(label = Gene), hjust = 1.1, size = 3, color = "white", fontface = "bold") +
  ...
  labs(title = "Top 3 Most Highly Expressed Genes per Replicate",
       x = "Raw Read Counts (Log10)", y = NULL)
```

The most abundant transcripts are consistent across samples within each cell line. Mitochondrial genes and *RNA45SN* dominate across samples. Crucially, *ONECUT2* appears as a top gene in overexpression samples, validating the experimental perturbation and analysis.

Given the prevalence of mitochondrial transcripts, lets quantify the exact proportion of the library they occupy per sample. Additionally, we can inspect the ribosomal content to confirm the efficiency of the Poly-A selection, ensuring that mature mRNA was successfully enriched.
...
ggplot(qc_summary, aes(x = Sample, y = Percentage, fill = Type)) +
  ...
  labs(title = "Library Composition Analysis", y = "Percentage of Total Reads", x = NULL)
```

PC3 samples have a significantly higher mitochondrial burden (\~45%) compared to LNCaP. This could mean a different metabolic profile or different response to the library preparation protocol between these cell lines. Note- its not a treatment or genetic perturbation effect as we see the similar mito content across all samples for PC3 samples.
