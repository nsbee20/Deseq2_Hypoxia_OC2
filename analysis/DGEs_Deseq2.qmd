...
***Figure 1:** Principal component analysis for LNCaP and PC3 samples. Genetic perturbation is dsiplayed by color and Treatment condition is displayed by different symbols.*
...
***Figure 5:** MA plot for both cell lines. Each dot represents a gene and differential expressed genes between normoxia and hypoxia conditions for empty vector and siCtrl for LNCaP and PC3 Cells, respectively with padj\<0.05 are shown in blue.*
...
***Figure 8:** ONECUT2 (OC2) expression in PC3 samples. Normalized count shown on x-axis. Rep1/ rep2 for siCtrl and two guides for siOC2 as shown as for siOC2 samples.*
...
***Figure 10:** Volcano plots displaying DEGs betwen siCtrl and siOC2 under both treatments.*

We can see some genes that change in the same direction. For example, CLDN1 is unregulated while NOX5 is down regulated. Lets do a sanity to check to confirm the results we are seeing are consistent with normalized counts.
...
# --- 2. DEFINE SIGNIFICANT GENES PER CONDITION ---
...
# --- 3. GENERATE NORMOXIA HEATMAP ---
mat_norm <- norm_counts[sig_genes_norm, norm_samples]
mat_norm_scaled <- t(scale(t(mat_norm)))

pheatmap::pheatmap(mat_norm_scaled, 
         cluster_cols = TRUE,   
         cluster_rows = TRUE,   
         show_rownames = FALSE, 
         show_colnames = TRUE,
         angle_col = 45,         
         fontsize_col = 10,      # Increases column label size
         main = paste("PC3 Normoxia:", length(sig_genes_norm), "Significant DEGs"),
         color = colorRampPalette(c("royalblue4", "white", "firebrick3"))(100),
         border_color = NA)

# --- 4. GENERATE HYPOXIA HEATMAP ---
mat_hypo <- norm_counts[sig_genes_hypo, hypo_samples]
mat_hypo_scaled <- t(scale(t(mat_hypo)))

pheatmap::pheatmap(mat_hypo_scaled, 
         cluster_cols = TRUE, 
         cluster_rows = TRUE, 
         show_rownames = FALSE, 
         show_colnames = TRUE,
         angle_col = 45,         
         fontsize_col = 10,      # Increases column label size
         main = paste("PC3 Hypoxia:", length(sig_genes_hypo), "Significant DEGs"),
         color = colorRampPalette(c("royalblue4", "white", "firebrick3"))(100),
         border_color = NA)
```
